...
function normalizeOptions(tree: Tree, options: HeroLibGeneratorSchema): GeneratorOptions {
  return {
    ...options,
...
function modifyItemsJson(tree: Tree, options: GeneratorOptions) {
  const targetProjectRoot = [...getProjects(tree).values()].find(
...
function generateAdditionalFiles(tree: Tree, options: GeneratorOptions) {
  ...
  const target = path.relative(
    path.join(tree.root),
    path.join(process.cwd(), options.directory, 'src', 'lib', options.componentName)
  );
